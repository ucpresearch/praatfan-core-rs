...
pub(crate) fn drfti1(wa: &mut [f64], ifac: &mut [i32]) {
    let n = wa.len() as i32;
    let mut nf = 0;
    ...
    ifac[1] = nf;
    let nfm1 = nf - 1;
    let argh = std::f64::consts::TAU / n as f64;
    let mut is = 0;
    let mut l1 = 1;
    ...
    for k1 in 0..nfm1 {
        ...
        while j < ipm {
            ...
            while ii < ido {
                ...
                let fresh0 = i;
                i += 1;
                wa[fresh0] = f64::cos(arg);
                let fresh1 = i;
                i += 1;
                wa[fresh1] = f64::sin(arg);
                ii += 2i32
            }
            is += ido as usize;
            j += 1
...
pub(crate) fn fdrffti(n: usize, wsave: &mut [f64], ifac: &mut [i32]) {
    if n == 1 {
        return;
    }
