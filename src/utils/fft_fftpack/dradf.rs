pub(crate) fn dradf2(
    ...
    for _ in 0..l1 {
        ...
        // Fix: speexdsp-rs's c2rust output mis-rendered Praat's original
        // `ch[(t1 << 1) + t3 - 1]` as `ch[t1 << (1 + t3 - 1)]`. Restored.
...
pub(crate) fn dradf4(
    ...
    const HSQT2: f64 = 0.70710678118654752440084436210485;
...
pub(crate) fn dradfg(
    ...
    let arg = std::f64::consts::TAU / ip as f64;
    let dcp = f64::cos(arg);
    let dsp = f64::sin(arg);
    let ipph = (ip + 1) >> 1;
    let ipp2 = ip;
